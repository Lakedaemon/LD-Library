...
\LD@Exo@Notion@Add{Forme alternee}{Forme alternee}%
...
\LD@Exo@Theme@Add{Logique}{Logique}%
...
\def\LD@Maths@Exo#1. #2\par{%
	...
	\EA\LD@String@Append\EA{\EA{\LD@Option@@Solution} Ind=}\LD@Temp
	\EA\LD@String@Append\EA{\EA{\LD@Option@@Indication} Notion=}\LD@Temp
...
\def\LD@Maths@Exercice#1{%
	...
	\ifcsname Data#1\endcsname
		...
		\LD@Data@Def{Ind}{#1}\LD@Exo@@Indication
...
\def\LD@Maths@Exercice@MetaData{%
	\smash{%
		\unless\ifx\LD@Exo@@Origin\LD@Empty
			...
			\ifx\LD@Test\LD@Empty
				\raise4pt\llap{\fivepts(\LD@Exo@@Origin)\quad}%
			\fi
		\fi
		\if Y\LD@Exo@Label@@Show
			\llap{$\underline{\hbox{{\eightpts \LD@Option@@Label}}}$\quad}%
		\fi
		\vbox{%
			\fivepts
			\llap{%
